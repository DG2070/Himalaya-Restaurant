...
export class AuthenticationService {
  ...
  async signUp(signUpDto: SignUpDto) {
    const [message, error] = await safeError(
      runInTransaction(async (queryRunner: QueryRunner) => {
        ...
        const [employeeRole, _error] = await safeError(
        ...
        if (!employeeRole)
          throw new NotFoundException(
            "Error assigning default 'employee' role to the user, check if the role exist."
          );
  ...
  async otpSignIn(oTPLoginDto: OTPLoginDto) {
    ...
    try {
      ...
      if (
        ...
        error instanceof RedisValueFetchException
      )
        throw error;
  ...
  async getLoginOTP(getSignInOTPDto: GetSignInOTPDto) {
    try {
      ...
      if (
        ...
        error instanceof RedisInsertionException
      )
        throw error;
      throw new InternalServerErrorException(
        `Error while getting Sign In OTP.`
      );
    }
  }
  ...
  async refreshTokens(refreshTokenDto: RefreshTokenDto) {
    try {
      ...
      if (
        error instanceof RedisValidationException ||
        error instanceof RedisInValidationException
      )
        throw error;
      throw new InternalServerErrorException(
        `Errr while refreshing the token. Try again.`
      );
    }
  }
